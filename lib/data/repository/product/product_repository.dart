...
class ProductRepository extends GetxController {
  ...
  Future<List<ProductModel>> fetchAllProducts() async {
    try {
      final query = await _db.collection(UKeys.productCollection).get();
  ...
  Future<ProductModel> fetchSingleProduct(String productId) async {
    try {
      final query = await _db.collection(UKeys.productCollection).doc(productId).get();
  ...
  Future<List<ProductModel>> fetchAllFeaturedProducts() async {
    try {
      final query = await _db.collection(UKeys.productCollection).where('isFeatured', isEqualTo: true).get();
  ...
  Future<List<ProductModel>> getProductsForBrand({required String brandId, int limit = -1}) async {
    try {
      final query = limit == -1
          ? await _db.collection(UKeys.productCollection).where('brand.id', isEqualTo: brandId).get()
          : await _db.collection(UKeys.productCollection).where('brand.id', isEqualTo: brandId).limit(limit).get();
  ...
  Future<List<ProductModel>> getProductsForCategory({required String categoryId, int limit = 4}) async {
    try {
      ...
      final productQuery = await _db.collection(UKeys.productCollection).where(FieldPath.documentId, whereIn: productIds).get();
  ...
  Future<List<ProductModel>> getFavouriteProducts(List<String> productsIds) async {
    try {
      final query = await _db.collection(UKeys.productCollection).where(FieldPath.documentId, whereIn: productsIds).get();
